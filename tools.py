...
# --- Resume / Cover-letter stubs (no resume mode yet) ---
class JobSearchInput(BaseModel):
    keywords: str = Field(..., description="Job title or keywords to search for")
    location_name: Optional[str] = Field(None, description="City / region to search in")
    job_type: Optional[str] = Field(None, description="onsite | remote | hybrid")
    limit: Optional[int] = Field(5, description="Max number of results")
    employment_type: Optional[str] = Field(None, description="full-time | contract | part-time etc.")
    listed_at: Optional[int] = Field(None, description="Posted within N seconds")
    experience: Optional[str] = Field(None, description="entry-level | mid-senior-level etc.")
    distance: Optional[int] = Field(None, description="Distance in miles from location")

def load_resume(path: str) -> str:
    raise NotImplementedError("Resume mode not enabled yet.")

def write_cover_letter_to_doc(content: str, filename: str) -> str:
    raise NotImplementedError("Cover letter mode not enabled yet.")
